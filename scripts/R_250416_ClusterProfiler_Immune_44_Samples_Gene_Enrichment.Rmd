---
title: "GSEA Analysis for Immune 44 Samples"
author: "Saumya"
date: "`r Sys.Date()`"
output:
  html_document:
...
library(msigdbr, lib.loc = "/hpc/users/pothus02/.Rlib")
...
library(ggplot2)
library(fgsea)
library(msigdbdf, lib.loc = "/hpc/users/pothus02/.Rlib")
```
...
# data.combined$subcluster <- Idents(data.combined)
# 
# # Set condition as active identity
# Idents(data.combined) <- "condition"
# data.combined <- JoinLayers(data.combined)
# # Create an empty list to store results
# markers_list <- list()
# 
# # Loop over each subcluster
# for (cluster in unique(data.combined$subcluster)) {
#   #Subset data for the current subcluster
#   message(paste0("Processing cluster: ", cluster))
#   subcluster_data <- subset(data.combined, subset = subcluster == cluster)
# 
#   # Set condition as active identity
...
#   markers_list[[paste0("immune_44_cluster_", cluster)]] <- markers
# }
# 
# # Save results to CSV
# for (name in names(markers_list)) {
#   write.csv(markers_list[[name]], file=paste0("../csvs/condition/", name, "_HIV_vs_Control.csv"))
# }
```
...
gsea_human <- function(df, condition){
  ...
  pathways_list <- split(human_gene_sets$gene_symbol, human_gene_sets$gs_name)
  ...
  gsea_result$p.adjust <- p.adjust(gsea_result$pval, method = "fdr")
...
csv_files <- list.files(csv_dir, pattern = "_HIV_vs_Control.csv", full.names = TRUE)
...
write.xlsx(combined_results, file = "../csvs/condition/immune_44_combined_GSEA_results.xlsx")
```

```{r, eval=FALSE, echo=FALSE, message=FALSE, warning=FALSE}
# Read the input CSV file
# degs_samples <- read_csv("../csvs/data_filt_44_all_markers_29_clusters.csv")
# 
...
# write.xlsx(results_list_samples, file = "../csvs/ClusterProfiler_GSEA_Immune_44_samples.xlsx")
# write.xlsx(gsea_results_samples, file = "../csvs/GSEA_Cluster_Info_Immune_44_Samples.xlsx")
```

```{r, echo=FALSE, message=FALSE, warning=FALSE}
# Read the sheets into data frames
samples_results <- read_excel("../csvs/condition/immune_44_combined_GSEA_results.xlsx")
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
# Function to plot up/down pathways safely
plot_top_pathways <- function(gsea_results, top_n = 10) {
  ...
  up_plot <- if (nrow(top_up) > 0) {
    ggplot(top_up, aes(x = reorder(pathway, NES), y = NES)) +
  ...
  down_plot <- if (nrow(top_down) > 0) {
    ggplot(top_down, aes(x = reorder(pathway, -NES), y = NES)) +
...
<!-- ## `r colorize('Barplots', 'blue')` {.tabset} -->
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
# PDF marker plot 
# pdf("pdf_clusterprofiler_samples.pdf", width = 10, height = 8, useDingbats = FALSE)
# print(plots_samples$up_plot)
# print(plots_samples$down_plot)
# dev.off()
```

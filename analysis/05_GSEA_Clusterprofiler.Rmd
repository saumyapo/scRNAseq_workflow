...
csv_dir <- "../../../csvs/myeloid/condition_DEGs/cluster_wise"
...
gsea_human <- function(df) {
  ...
  lfc_vector <- df$avg_log2FC
  names(lfc_vector) <- df$gene
  lfc_vector <- sort(lfc_vector, decreasing = TRUE)
...
```{r}
gsea_dir <- "../../../csvs/myeloid/condition_GSEA/"
pdf_dir <- "../../../pdfs/myeloid/condition_GSEA/"
...
for (cmp in pairwise_comparisons) {
  ...
  message("Processing GSEA for: ", cond1, " vs ", cond2)
  ...
  if (length(all_gsea_results) > 0) {
    ...
    write.xlsx(
      ...
      file = file.path(
        gsea_dir,
        paste0("filt_myeloid_subset_GSEA_", cond1, "_vs_", cond2, ".xlsx")
      )
    )
  }
}
...
```{r}
# Directory containing GSEA excel files
library(readxl)
library(dplyr)
library(openxlsx)
library(stringr)

input_dir <- "../../../csvs/myeloid/condition_GSEA/"
output_dir <- input_dir

# Find all GSEA Excel files automatically
files <- list.files(
  input_dir,
  pattern = "GSEA.*\\.xlsx$",
  full.names = TRUE
...
for(file in files){
  ...
  gsea <- read_excel(file)
  
  # Split by subcluster
  subcluster_list <- split(gsea, gsea$subcluster)
  
  wb <- createWorkbook()
  
  for(sub in names(subcluster_list)){
    
    df <- subcluster_list[[sub]]
    ...
    sheet_name <- str_extract(sub, "(?<=cluster_).+?(?=_CAR-T Enterocolitis|_CAR-T Control|_NV)")
    cond <- case_when(
      grepl("CAR-T Enterocolitis", sub) ~ "EC",
      grepl("CAR-T Control", sub)       ~ "CTRL",
      grepl("_NV", sub)                 ~ "NV"
    )
    sheet_name <- paste0(sheet_name, "_", cond)
    sheet_name <- substr(sheet_name, 1, 31)
    if(is.na(sheet_name)) sheet_name <- "all_clusters"
    
    addWorksheet(wb, sheet_name)
    
    writeData(wb, sheet_name, df_filt)
...
plot_nes_dotplot_dual <- function(gsea_df,
                                  subclusters = NULL,
                                  pathways = NULL,
                                  comparisons = NULL,
                                  top_n = 15) {
  ...
  if (!is.null(subclusters)) {
    gsea_df <- gsea_df %>% dplyr::filter(subcluster %in% subclusters)
  }
  if (!is.null(comparisons)) {
    gsea_df <- gsea_df %>% dplyr::filter(comparison %in% comparisons)
  }
  
  df_sig <- gsea_df %>% dplyr::filter(qvalue < 0.05)
  
  if (nrow(df_sig) == 0) {
    return(ggplot() + annotate("text", x=0, y=0, label="No significant pathways") + theme_void())
  }
  
  if (!is.null(pathways)) {
    df_sig <- df_sig %>% dplyr::filter(Description %in% pathways)
  } else {
    df_sig <- df_sig %>%
      group_by(subcluster, comparison) %>%
      { bind_rows(
        filter(., NES > 0) %>% arrange(desc(NES)) %>% head(top_n),
        filter(., NES < 0) %>% arrange(NES) %>% head(top_n)
      )} %>%
      ungroup()
  }
  
  # Clean labels and reorder
  df_sig <- df_sig %>%
    mutate(label = clean_pathway_names(Description))
  
  df_sig$label <- factor(
    df_sig$label,
    levels = unique(df_sig$label[order(df_sig$NES)])
  )
  
  
  subtitle_text <- unique(df_sig$subcluster) %>%
    gsub("stromal_subset_cluster_", "", .) %>%
    gsub("_CAR-T.*", "", .)
  
  ggplot(df_sig, aes(x = NES, y = label, size = setSize, color = qvalue)) +
    geom_point(alpha = 0.85) +
    facet_wrap(~comparison) +
    scale_color_gradientn(
      colors = c("navy", "orchid3", "firebrick"),   
      trans  = "reverse",
      name   = "Adj. p-value",
      guide  = guide_colorbar(barwidth = 0.7, barheight = 5, ticks = TRUE)
    ) +
...
```{r}
gsea_dir <- "../../csvs/epithelial/condition_GSEA/"
pdf_dir <- "../../pdfs/epithelial/condition_GSEA/"

# gsea_ec_nv <- read_excel(
#   file.path(gsea_dir,
#             "epithelial_subset_GSEA_EC_vs_NV.xlsx")
# ) %>% as.data.frame()
# 
# gsea_ec_nv$comparison <- "Enterocolitis vs NV"

gsea_ctrl_nv <- read_excel(
  file.path(gsea_dir,
            "epithelial_subset_GSEA_CTRL_vs_NV.xlsx")
) %>% as.data.frame()

gsea_ctrl_nv$comparison <- "Control vs NV"
 
 combined_results <- bind_rows(gsea_ec_nv, gsea_ctrl_nv)
...
p <- plot_nes_dotplot_dual(
  gsea_df = gsea_ctrl_nv,
  subclusters = "epithelial_subset_cluster_Mature colonocytes_CTRL_vs_NV",
  pathways = pathways_interest,
  comparisons = c("Control vs NV")
)

...
# Stacked bar plot with no p-values

```{r}
meta <- data.combined@meta.data %>%
  filter(condition %in% c("NV", "UC"))
...
summary_avg <- per_sample_counts %>%
  group_by(condition, cluster_label) %>%
...
colour_palette <- c("#E41A1CFF", "#377EB8FF", "#4DAF4AFF", "#984EA3FF", "#FF7F00FF", "#FFFF33FF", "#A65628FF", "#F781BFFF", "#999999FF")

names(colour_palette) <- sort(unique(summary_avg$cluster_label))

p <- ggplot(summary_avg, aes(x = condition, y = mean_pct, fill = cluster_label)) +

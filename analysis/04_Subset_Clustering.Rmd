...
```{r, eval = FALSE, echo=FALSE, message=FALSE, warning=FALSE}
# Read Seurat object
filt_myeloid_subset <- readRDS("../../../RDS/myeloid/filt_myeloid_subset_umap_15_clusters.RDS")

Idents(filt_myeloid_subset) <- "condition"
filt_myeloid_subset <- JoinLayers(filt_myeloid_subset)

# Explicit directional comparisons
...
for (cmp in pairwise_comparisons) {
  ...
  ident1 <- cmp[1]  # condition of interest
  ident2 <- cmp[2]  # baseline

  # Skip if either condition missing
  if (!all(c(ident1, ident2) %in% unique(Idents(filt_myeloid_subset)))) {
    next
  }

  message(paste0("Running all-cluster DEGs: ", ident1, " vs ", ident2))
  ...
  # Add cluster label
  overall_df <- as.data.frame(data.markers)
  overall_df$cluster <- "all"
  overall_df <- overall_df[, c("gene", setdiff(colnames(overall_df), "gene"))]

  # Store
  list_name <- paste0("all_", ident1, "_vs_", ident2)
  overall_markers_list[[list_name]] <- overall_df

  # Save CSV (matches your existing convention)
  write.csv(
    overall_df,
    file = paste0(
      "../../../csvs/myeloid/condition_DEGs/cluster_wise/",
      "filt_myeloid_subset_all_",
      ident1, "_vs_", ident2,
      ".csv"
    ),
    row.names = FALSE
...
DefaultAssay(filt_myeloid_subset) <- "RNA"
Idents(filt_myeloid_subset) <- "cluster_label"

# Store subcluster identities
filt_myeloid_subset$subcluster <- Idents(filt_myeloid_subset)


filt_myeloid_subset <- JoinLayers(filt_myeloid_subset)

# Condition vs Control (e.g, UC vs NV)
pairwise_comparisons <- list(
  c("CAR-T Enterocolitis", "CAR-T Control"),  # colitis vs control
  c("CAR-T Enterocolitis", "NV"),             # colitis vs NV
  c("CAR-T Control", "NV")                    # control vs NV
)
...
for (cluster in unique(filt_myeloid_subset$subcluster)) {
  message(paste0("Processing cluster: ", cluster))
  

  subcluster_data <- subset(filt_myeloid_subset, subset = subcluster == cluster)
  
  # if (length(unique(Idents(subcluster_data))) < 2) {
  #   message(paste0("Skipping cluster ", cluster, " (not enough conditions present)"))
  #   next
  # }
  ...
  for (cmp in pairwise_comparisons) {
    ident1 <- cmp[1]  # condition of interest
    ident2 <- cmp[2]  # baseline condition
    
    # Skip if either condition is missing in this subcluster
    if (!all(c(ident1, ident2) %in% unique(Idents(subcluster_data)))) {
      message(paste0("Skipping comparison ", ident1, " vs ", ident2, " in cluster ", cluster))
      next
    }
    
    message(paste0("  Comparing: ", ident1, " vs ", ident2))
    ...
    # Store results
    list_name <- paste0("filt_myeloid_subset_cluster_", gsub("/", "-", cluster), "_", ident1, "_vs_", ident2)
...
for (name in names(markers_list)) {
  write.csv(
    markers_list[[name]],
    file = paste0("../../../csvs/myeloid/condition_DEGs/cluster_wise//", name, ".csv"),
    row.names = FALSE
  )
}
```
...
```{r}
# Output directory
agg_dir <- "../../../csvs/myeloid/condition_DEGs/"

# Extract comparison names from markers_list
# Keep everything after the cluster identifier
comparisons <- unique(
  sub("^.*cluster_[^_]+_", "", names(markers_list))
)

# Loop over each comparison
for (cmp in comparisons) {

  message(paste0("Aggregating comparison: ", cmp))

  # Keys for this comparison
  cmp_keys <- grep(paste0("_", cmp, "$"), names(markers_list), value = TRUE)

  dfs_all_clusters <- lapply(cmp_keys, function(k) {

    df <- as.data.frame(markers_list[[k]])
  ...
  all_markers_df <- do.call(rbind, dfs_all_clusters)

  # Preserve cluster order as factor
  all_markers_df$cluster <- factor(
    all_markers_df$cluster,
    levels = unique(all_markers_df$cluster)
  )

  
  all_markers_df <- all_markers_df[, c("gene", setdiff(colnames(all_markers_df), "gene"))]

  # Build sheets
  sheets_list <- list()

  for (cl in levels(all_markers_df$cluster)) {
    # Excel sheet name limit 31 characters
    safe_name <- substr(cl, 1, 31)
    sheets_list[[safe_name]] <- all_markers_df[all_markers_df$cluster == cl, , drop = FALSE]
  }

  # Optional combined sheet
  sheets_list[["all_clusters_combined"]] <- all_markers_df

  # Write workbook
  out_file <- file.path(
    agg_dir,
    paste0("filt_myeloid_subset_", substr(cmp,1,50), "_aggregated.xlsx") 
  )
  ...
  message(paste0("   written: ", out_file))
}
